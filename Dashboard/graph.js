...
saveDefBtn.on("click", () => {
  ...
  const hasIncoming = (data.links || []).some(l => {
  ...
  // In case the DOM changed, re-grab the value element if it was missing at boot
  const valueEl = nodeBaselineValueEl || document.getElementById("nodeBaselineValue");
  if (valueEl) valueEl.textContent = String(curBaseline);
  ...
  if (nodeBaselineRange){
    ...
    // Live update + commit on slider move (no manual apply)
    nodeBaselineRange.oninput = (ev) => {
      const v = safeNumber(ev.target.value, 0);

      // Update UI readout
      const valueEl2 = nodeBaselineValueEl || document.getElementById("nodeBaselineValue");
      if (valueEl2) valueEl2.textContent = String(v);

      // Commit baseline immediately
      baselineMap.set(node.id, v);
      const orig = data.nodes.find(n => n.id === node.id);
      if (orig) orig.baseline = v;

      // If it's an Input node, keep its slider default and current state aligned
  ...
  function showEdgeDetails(edge) {
    // IMPORTANT:
    // In setupGraph() we bind D3 to a shallow-cloned link object so that
    // we can swap source/target ids for node objects without mutating the
    // underlying model. That means clicks hand us the *clone*.
    //
    // For editing + exporting we must always operate on the canonical
    // edge object in `data.links`.
    const canonical = (data.links || []).find(l => l && l.id === edge.id);
    edge = canonical || edge;
    ...
    // Render a stable title using node labels from the canonical node list.
    // (The canonical edge uses string ids; the D3-bound clone may contain node objects.)
    const srcId = (typeof edge.source === "object" && edge.source) ? edge.source.id : edge.source;
    const tgtId = (typeof edge.target === "object" && edge.target) ? edge.target.id : edge.target;
    const srcLabel = (data.nodes || []).find(n => n && n.id === srcId)?.label || srcId;
    const tgtLabel = (data.nodes || []).find(n => n && n.id === tgtId)?.label || tgtId;

    drawerTitle.text(`${srcLabel} → ${tgtLabel}`);
    drawerEdge.text(`${srcLabel} → ${tgtLabel}`);
...
renderEdgeVars(edge);
  ...
  function exportModel() {
    ...
    // Also bake the current edge parameter values back into each edge's
    // vars[].default so the exported JSON behaves the same even without
    // the runtime block.
    if (Array.isArray(out.links)) {
      for (const l of out.links) {
        const state = out.runtime.edgeParamState?.[l.id];
        if (!state || !Array.isArray(l.vars)) continue;
        for (const v of l.vars) {
          if (v && v.name in state) v.default = state[v.name];
        }
      }
    }
