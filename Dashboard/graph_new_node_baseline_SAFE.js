...
for (const node of inputNodes) {
  ...
  // --- Node baseline/initial value editor ------------------------------------
  // Shows a slider for ANY node when it is clicked, allowing you to change its baseline (initial) value.
  // This affects delta-style formulas (fromDelta) because fromDelta = current - baseline.
  function ensureNodeBaselineEditorContainer() {
    let el = document.querySelector("#nodeBaselineEditor");
    if (el) return el;

    const right = document.querySelector("#right") || document.body;
    const sliders = document.querySelector("#sliders");
    el = document.createElement("div");
    el.id = "nodeBaselineEditor";
    el.className = "slider-card";
    el.style.display = "none";
    el.style.margin = "10px 14px";

    // Insert above the main input sliders list (if present)
    if (right && sliders && sliders.parentNode === right) {
      right.insertBefore(el, sliders);
    } else if (right) {
  ...
  function baselineRangeForNode(node) {
    // If the node already has a slider config, reuse it.
    if (node.slider && node.slider.min !== undefined && node.slider.max !== undefined) {
      return {
        min: node.slider.min,
        max: node.slider.max,
        step: node.slider.step ?? 1
      };
    }
    const b = safeNumber(node.baseline, 0);
    const span = Math.max(100, Math.abs(b) || 0);
    return { min: b - span, max: b + span, step: span / 100 };
  }

  function showNodeBaselineEditor(node) {
    const el = ensureNodeBaselineEditorContainer();
    if (!node || !el) return;

    const unit = node.unit ? ` (${node.unit})` : "";
    const currentBaseline = safeNumber(getBaseline(node.id), safeNumber(node.baseline, 0));
    const rng = baselineRangeForNode(node);

    el.style.display = "block";
    el.innerHTML = `
      <div class="slider-title">Selected: ${escapeHtml(node.label || node.id)}</div>
      <div class="slider-subtitle">Initial / baseline value${escapeHtml(unit)}</div>
      <div class="slider-row">
        <input id="nodeBaselineSlider" type="range" min="${rng.min}" max="${rng.max}" step="${rng.step}" value="${currentBaseline}">
        <div class="slider-value" id="nodeBaselineValue"></div>
      </div>
      <div class="tiny">Tip: this changes the baseline used by delta formulas (fromDelta). Export model to save.</div>
    `;
    ...
    const valueEl = el.querySelector("#nodeBaselineValue");
    const slider = el.querySelector("#nodeBaselineSlider");
    if (valueEl) valueEl.textContent = fmt(currentBaseline);
    ...
    if (slider) {
      slider.addEventListener("input", (ev) => {
        ...
        // Update baseline map + node baseline (so export picks it up)
        baselineMap.set(node.id, v);
        node.baseline = v;

        // If this node is an input slider node, keep its current value aligned with baseline
        if (node.slider) {
          node.slider.default = v;
          inputState[node.id] = v;
          renderInputSliders();
        }
  ...
  function hideNodeBaselineEditor() {
    const el = document.querySelector("#nodeBaselineEditor");
  ...
  function setupGraph() {
    ...
    nodeSel = g.append("g").attr("class", "nodes")
      ...
      .on("click", (ev, d) => {
        ...
        // NEW: show baseline/initial value slider for this node
        showNodeBaselineEditor(d);
