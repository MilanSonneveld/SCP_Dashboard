...
saveDefBtn.on("click", () => {
  ...
  if (nodeBaselineInput) nodeBaselineInput.value = String(safeNumber(getBaseline(node.id), 0));

  applyNodeBaselineBtn.on("click", () => {
    if (!nodeBaselineInput) return;
    const v = safeNumber(nodeBaselineInput.value, 0);
